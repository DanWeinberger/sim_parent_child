...
sirHH <- function( times=60, 
                   ...
                   logit.mu=logit(c(1/20,1/20)), #waning of protection from subsequent infection
  ...
  for(i in 2:times){
   for(j in 1:2){
    ...
    prob_S <- prob_escape_infect+
                 X[j,c('R'),i-1]*(mu[j])  #Susceptible person Escape infection from HH if HH member is infected
    ...
    prob_R <- X[j,c('I'),i-1]*(delta[j]) +
                 X[j,c('R'),i-1]*(1-mu[j]) #if R=1 at previous period, stay in 
